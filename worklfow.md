...
3. Si la PR est ok, je peux merger dans `master` : [ci.yml](/.github/workflows/ci.yml)
   est à nouveau lancé
...
`$CI_REGISTRY/cour-de-cassation/judilibre-admin/<environnement>:<sha>`.
...
Cette partie est inchangée. Le cluster Scaleway ne peut pas atteindre le registry
GitLab interne : il tire son image depuis Docker Hub.

1. Je crée un tag sur `master`. Le workflow [tag.yml](/.github/workflows/tag.yml)
   construit l'image et la pousse sur Docker Hub
2. Le tag est répliqué sur le GitLab interne par le mirroring, ce qui y déclenche un
   pipeline. Le job `check_if_image_is_on_dockerhub` attend que l'image soit
   disponible sur Docker Hub (12 tentatives espacées de 5 secondes)
3. Une fois l'image trouvée et la preprod privée déployée, `deploy_staging` peut être
   déclenché manuellement. Il ne redéploie pas les ressources : il met simplement à
   jour le nom de l'image dans le cluster Kubernetes
4. Le déploiement de la production publique reste à écrire (bloc commenté en fin de
