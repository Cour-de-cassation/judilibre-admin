...
3. `image` : laissée **vide**, l'image du commit est déployée — construite et
   publiée sous `<organisation>/judilibre-admin:<sha court>` si le registre ne la
   porte pas déjà. **Renseignée**, cette référence est déployée telle quelle, sans
   construction : c'est ce qui permet de redéployer une image déjà en service ou
...
le LoadBalancer et l'authentification, ce que les sondes du conteneur ne prouvent
pas. Si le déploiement ou ce contrôle échoue, le Deployment revient à sa révision
précédente.
...
Les trois environnements GitHub `scw-dev-par1`, `scw-prod-par1` et `scw-prod-par2`
portent les règles de déploiement. Les deux environnements de production
n'acceptent que la branche `master`.

### Le tag, côté public

[tag.yml](/.github/workflows/tag.yml) est indépendant de ce qui précède : un tag
`X.Y.Z` posé sur `master` construit et publie
`<organisation>/judilibre-admin:<tag>` sur Docker Hub, sans déclencher aucun
déploiement.
